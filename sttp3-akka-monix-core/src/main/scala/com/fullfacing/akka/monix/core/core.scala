...
import sttp.client3.{ByteArrayBody, ByteBufferBody, FileBody, InputStreamBody, RequestBody, StringBody}
...
package object core {
  ...
  def createMultiPartRequest(mps: Seq[Part[RequestBody[_]]], request: HttpRequest): Either[Throwable, HttpRequest] = {
  ...
  def convertMultiPart(mp: Part[RequestBody[_]]): Either[Throwable, FormData.BodyPart] = {
    for {
  ...
  def createBodyPartEntity(ct: ContentType, body: RequestBody[_]): BodyPartEntity = body match {
